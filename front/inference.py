...
class Inference:
    def __init__(
        ...
        self.imgszs = [640, 1024]
        
        self.models = [YOLO(path).to("mps") for path in models]

    def tta(self, model, image: np.ndarray, imgsz: int, iou: float = 0.7, conf: float = 0.1) -> list[list[float]]:
        return
    
    def infer(self, img_path) -> torch.Tensor:
        if isinstance(img_path, str):
            image = cv2.imread(img_path)
        else:
            image = img_path
        image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
        out_ = self.predict(image)

        boxes:list[list[float]] = out_[0]
        scores:list[float] = out_[1]
        ...
        for idx, bbox in enumerate(boxes):
            if scores[idx] > 0.01:
                ...
                y1 *= W
                y2 *= W
                bboxes_output.append([x1, y1, x2, y2, scores[idx]])
        return torch.Tensor(bboxes_output)

    def predict(self, image: np.ndarray, iou: float = 0.7, conf: float = 0.1) -> tuple[list[list[float]] | list[float]]:
        '''
